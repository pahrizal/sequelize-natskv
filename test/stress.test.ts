...
import { NatsKVSequelize } from "../src";
...
describe("NATS KV ORM Stress Test", () => {
  ...
  function logUsage(
    ...
    console.log(
      `${label} - Memory Δ: rss ${memDiff.rss.toFixed(
        2
      )}MB, heapUsed ${memDiff.heapUsed.toFixed(2)}MB, CPU Δ: user ${
        cpuDiff.user
      }ms, system ${cpuDiff.system}ms`
    );
  }
  ...
  it(`should handle bulk create, read, update, and delete for ${USER_COUNT} users`, async () => {
    ...
    const t6 = Date.now();
    await Promise.all(
      users.map((user) => User.update({ age: 99 }, { where: { id: user.id } }))
    );
    const t7 = Date.now();
    ...
    const t8 = Date.now();
    await Promise.all(
      users.map((user) => User.destroy({ where: { id: user.id } }))
    );
    const t9 = Date.now();
